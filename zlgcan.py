...
def zlgcan_init():
    ...
    for i in range(transmit_num):
        ...
        msgs[i].frame.can_id = 0x500
        msgs[i].frame.can_dlc = 8
    ret = zcanlib.Transmit(chn_handle, msgs, transmit_num)
    print("Tranmit CANFD Num: %d." % ret)
    return handle, zcanlib, chn_handle

def zlgcan_nmsend(ZCAN_DEVICE_TYPE, chn_handle, can_id, can_lib):
    transmit_num = 1
    msgs = (ZCAN_Transmit_Data * transmit_num)()
    for i in range(transmit_num):
        msgs[i].Flags = 0
        msgs[i].TimeStamp = 6400  # 帧接收或者发送时的时间戳，单位为10us
        msgs[i].frame.can_id = can_id
        msgs[i].frame.can_dlc = 8
        msgs[i].frame.data[0] = 0x00
        msgs[i].frame.data[1] = 0x40
        msgs[i].frame.data[2] = 0x04
        msgs[i].frame.data[3] = 0x00
        msgs[i].frame.data[4] = 0x00
        msgs[i].frame.data[5] = 0x00
        msgs[i].frame.data[6] = 0x00
        msgs[i].frame.data[7] = 0x00
    ret = can_lib.Transmit(chn_handle, msgs, transmit_num)
    return ret

# 用于发送测距启停报文
def zlgsend_cmd_msg_fd(ZCAN_DEVICE_TYPE, chn_handle, can_id, byte1, zcanlib):
    transmit_num = 1
    msgs = (ZCAN_TransmitFD_Data * transmit_num)()
    for i in range(transmit_num):
        msgs[i].transmit_type = 0  # 正常发送
        msgs[i].frame.eff = 0      # 标准帧
        msgs[i].frame.rtr = 0      # 数据帧
        msgs[i].frame.can_id = can_id
        msgs[i].frame.len = 64     # 发送64字节
        msgs[i].frame.brs = 0
        msgs[i].frame.esi = 0
        for j in range(64):
            msgs[i].frame.data[j] = 0x00
        msgs[i].frame.data[0] = 0x24
        msgs[i].frame.data[1] = byte1
    ret = zcanlib.TransmitFD(chn_handle, msgs, transmit_num)
    print("发送CANFD报文ID: %02X" % can_id)
    print("数据:", [hex(msgs[0].frame.data[j]) for j in range(64)])
    return ret

def zlgcan_send(chn_handle, can_id, can_lib, dlc, data, frameInterval, sendMsgNum):
    # if dlc > 8:
    msgs = (ZCAN_TransmitFD_Data * sendMsgNum)()
    for i in range(0, sendMsgNum):
        msgs[i].Flags = 0
        msgs[i].TimeStamp = frameInterval  # 帧接收或者发送时的时间戳，单位为10us
        msgs[i].frame.can_id = can_id
        # msgs[i].frame.can_dlc = dlc
        msgs[i].frame.len = dlc
        for j in range(0, dlc):
            # print("frame data len:", len(msgs[i].frame.data))
            # print("data len:", len(data))
            msgs[i].frame.data[j] = data[j]
    ret = can_lib.TransmitFD(chn_handle, msgs, sendMsgNum)
    return ret
    # else:
    #     msgs = (ZCAN_Transmit_Data * sendMsgNum)()
    #     for i in range(0, sendMsgNum):
    #         msgs[i].Flags = 0
    #         msgs[i].TimeStamp = frameInterval  # 帧接收或者发送时的时间戳，单位为10us
    #         msgs[i].frame.can_id = can_id
    ...
    #             # print("data len:", len(data))
    #             msgs[i].frame.data[j] = data[j]
    #     ret = can_lib.Transmit(chn_handle, msgs, sendMsgNum)
    #     return ret

# 用于读取报文
def ZLGread_msg(chn_handle, zcanlib):
    msgs = []
    # 读取标准CAN报文
    rcv_num = zcanlib.GetReceiveNum(chn_handle, ZCAN_TYPE_CAN)
    if rcv_num > 0:
        rcv_msgs, actual_num = zcanlib.Receive(chn_handle, rcv_num)
        for i in range(actual_num):
            msg = {
                "type": "CAN",
                "can_id": rcv_msgs[i].frame.can_id,
                "dlc": rcv_msgs[i].frame.can_dlc,
                "data": [rcv_msgs[i].frame.data[j] for j in range(rcv_msgs[i].frame.can_dlc)],
                "timestamp": rcv_msgs[i].timestamp
            }
            msgs.append(msg)
    # 读取CANFD报文
    rcv_fd_num = zcanlib.GetReceiveNum(chn_handle, ZCAN_TYPE_CANFD)
    if rcv_fd_num > 0:
        rcv_fd_msgs, actual_fd_num = zcanlib.ReceiveFD(chn_handle, rcv_fd_num)
        for i in range(actual_fd_num):
            msg = {
                "type": "CANFD",
                "can_id": rcv_fd_msgs[i].frame.can_id,
                "len": rcv_fd_msgs[i].frame.len,
                "data": [rcv_fd_msgs[i].frame.data[j] for j in range(rcv_fd_msgs[i].frame.len)],
                "timestamp": rcv_fd_msgs[i].timestamp
            }
            msgs.append(msg)

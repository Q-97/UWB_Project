...
class RadarDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return torch.tensor(self.features[idx]), torch.tensor(self.labels[idx]), idx
...
def train_one_seed(data_dir, seed, save_dir):
    ...
    model = RadarMLP(T_WINDOW * M_POINTS * 2).to(device)
    # --- 修改 Loss 选择逻辑 ---
    if LOSS_TYPE == "Focal":
...
if __name__ == "__main__":
    ...
    folder_name = f"Res_{loss_str}_T{T_WINDOW}_M{M_POINTS}_B{BATCH_SIZE}_LR{LEARNING_RATE:.5f}_E{EPOCHS}_X{X_MIN}_{X_MAX}_Y{Y_MIN}_{Y_MAX}"
